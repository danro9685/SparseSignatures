...
\begin{figure*}[ht]
\begin{center}
\includegraphics[width=0.5\textwidth]{figure/image-1-1}
\end{center}
...
First of all, we need to specify the ranges of number of signatures (variable K) and the LASSO penalty value (variable lambda rate) to be considered. In particular, the latter is more complicated to estimate, as it requires that the values in the range are not too small in order to avoid dense signatures, but also they should not be to high in order to still perform a good fit of the observed counts. 
