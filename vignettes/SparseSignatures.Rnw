...
Those data are a reduced version with only 3 patients of the 560 breast tumors provided by Nik-Zainal, Serena, et al. (2016). We can transform such input data to a count matrix to perform the signatures discovery with the function import.data. To do so, we also need to specify the reference genome as a BSgenome object and the format of the 96 nucleotides to be considered. This can be done as follow, where in the example we use hs37d5 as our reference genome. 
...
The function import.data can also take a file as input with the same format as the one shown above. Now, we show an example of visualization feature provided by the package, and we show the counts for the first patient PD10010a in the following plot. 
...
\begin{figure*}[ht]
\begin{center}
\includegraphics[width=0.7\textwidth]{figure/image-1-1}
\end{center}
...
First of all, we need to specify the ranges for the number of signatures (variable K) and the LASSO penalty value (variable lambda rate) to be considered. In particular, the latter is more complicated to estimate, as it requires that the values in the range not to be too small in order to avoid dense signatures, but also they should not be to high in order to still perform a good fit of the observed counts. 
...
First, we can estimate the initial values of beta as follow. 

<<starting_betas,eval=FALSE>>=
starting_betas = starting.betas.estimation(x=patients,K=3:12,background_signature=background)
@

Then, we also need to explore the search space of values for LASSO penalty in order to make a good choice. To do so, we can use the function evaluate.lambda.range to test different values as follows. 

<<lambda_range,eval=FALSE>>=
lambda_range = evaluate.lambda.range(x=patients,K=10,beta=starting_betas[[8,1]],lambda_values=c(0.05,0.10))
@
...
To evaluate the best lambda range, we need to carefully consider the log-likelihood of the solutions at each iteration of our method. This can be done by exploiting the as. functions that we provide. Here some examples. 

<<example_small>>=
# example of too small value of lambda
...
\end{center}
\caption{Example of too small value of lambda: the log-likelihood is very unstable across the iterations.}
\end{figure*}

<<example_big>>=
# example of too high value of lambda
...
\end{center}
\caption{Example of too high value of lambda: the log-likelihood drops after the first iteration.}
\end{figure*}

<<example_good>>=
# example of good value of lambda
...
\end{center}
\caption{Example of good value of lambda: the log-likelihood is increasing across the iterations.}
\end{figure*}
...
We notice that the computations for this task can be very time consuming, expecially when a lot of iterations of cross validations are specified (see manual) and a large set of configurations of the parameters are tested. To speed up the executions, we suggest to use the parallel execution options. Also, to reduce the memory requirements, we advice to split the cross validation in different runs, e.g., if one wants to perform 100 iterations, we would suggest to make 10 independent runs of 10 iterations each. Also in this case, we provide as examples together with the package a set of pre-computed results obtained with the above command and the following settings: K = 3:10, cross validation entries = 0.10, cross validation iterations = 5, lambda values = c(0.05,0.10,0.15), num iterations cv = 2. 

<<example_data_cv>>=
data(cv_example)
@

We can now estimate the best configuration of the parameters in terms of median mean squared error by cross validation, where the best configuration is the one with lower error. 
...
Finally, we can now compute the signeture for the best configuration, i.e., K = 5 and lamnda = 0.10. 

<<best_configuation_signatures>>=
beta = starting_betas_example[["5_signatures","Value"]]
res = nmf.LassoK(x=patients,K=5,beta=beta,background=background,lambda_rate = 0.10,iterations=5,num_processes=NULL)
@
