...
load(file="data/signatures_nmfLasso_germline_10.RData")
...
for(i in 1:num_iterations) {
    ...
    stability_cv_10[[i]] = signatures_nmfLasso_germline[["best_configuration"]]
}
save(stability_cv_10,file="data/stability_cv_10.RData")

avg_K = NULL
avg_lambda_rate = NULL
for(i in 1:length(stability_cv_10)) {
    avg_K = c(avg_K,stability_cv_10[[i]][["K"]])
    avg_lambda_rate = c(avg_lambda_rate,stability_cv_10[[i]][["lambda_rate"]])
}
